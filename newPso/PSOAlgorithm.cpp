...
PSO::PSO(psoParam input) {
	...
	bSol = new psoSol[input.MaxIt];
	swarm = new psoPart[input.nPop];
	...
	for (int i = 0; i < nPop; i++) 
	{
		...
		// Initialize position first particle
		if(i == 0)
		{
		...
		cout << "\t\tServo Gain Parameters. Pn100: " << swarm[i].Position[0] << ", Pn101: " << swarm[i].Position[1] <<
			", Pn102: " << swarm[i].Position[2] << " Pn401: " << swarm[i].Position[3] << endl;
...
psoSol PSO::mainLoop() {
	for (int it = 1; it <= maxIt; it++) 
	{
		...
		cout << "\tGeneration: " << it << ": Best Cost = " << bSol[it].Cost << endl;
		cout << "\tBest Parameters. Pn100: " << bSol[it].Position[0] << " Pn101: " << bSol[it].Position[1] <<
			" Pn102: " << bSol[it].Position[2] << " Pn401: " << bSol[it].Position[3] << endl; //ICLi
		cout << "------------------------------" << endl;
...
int PSO::saveData(string filenameBest, string filenameData, string filenameTheBest) {
	...
	// Save best solution //Roxas
	fileTheBest.open(filenameTheBest);

	//fileTheBest<< (int)bSol[lim-1].Position[0] << "," << (int)bSol[lim-1].Position[1] << "," << (int)bSol[lim-1].Position[2] << "," << (int)bSol[lim-1].Position[3]<<endl;
	fileTheBest << (int)gBest.Position[0] << "," << (int)gBest.Position[1] << "," << (int)gBest.Position[2] <<
		"," << (int)gBest.Position[3] << "," << (int)gBest.Cost << endl;
...
void PSO::readData(string filenameGain, string filenameCost) {
	// File pointer 
	fstream finG, finC;

	// Open an existing file 
	finG.open(filenameGain, ios::in);
